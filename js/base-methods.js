...
function updateImageSrc(imageElement, imageType, imageNumber) {
    const template = imageElement.dataset.srcTemplate;

    if (!template) {
        console.warn('Відсутній data-src-template для елемента', imageElement);
        return;
    }

    const finalSrc = template
        .replace('{type}', imageType)
        .replace('{number}', imageNumber);

    imageElement.setAttribute('src', finalSrc);
}
