...
function renderTable(settings, horizontalSumItems, verticalSumItems) {
    ...
    renderList(
        ...
        _answersInput,
        (item, element, index) => {
            element.id = `answersInput_${index}`; // ← Унікальний id
            _updateAnswerInput(item, element);
        },
    )
}
...
function _updateAnswerInput(item, element) {
    // вважаємо “порожнім” лише якщо number === null або === undefined
    const isEmpty = item.number === null || item.number === undefined;
