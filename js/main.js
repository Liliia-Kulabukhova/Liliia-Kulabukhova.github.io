...
function _getItemsWithCheckedSum(
    ...
    const neverUsedUniqueItemIndexes = uniqueItems
        .map(
            (item, index) => item.usageCount === 0 ? index : -1
        )
    ...
    while (!uniqueSumFound) {
        ...
        for (let i = 0; i < size; i++) {
            ...
            newItems.push(uniqueItems[randomItemIndex]);
        }
        ...
        newItems = newItems.map((item, index) => {
            if (isRow && currentItems[currentIndex][index].number) {
                return currentItems[currentIndex][index];
            }

            if (!isRow && currentItems[index][currentIndex].number) {
                return currentItems[index][currentIndex];
            }
        ...
        // Check if sum is unique or the same
        if ((isSameSum && sums.includes(sum)) || (!isSameSum && !sums.includes(sum))) {
            uniqueSumFound = true;
            sums.push(sum);
            _updateUniqueItemsUsageCount(
                emptyCellItems.map((item) =>
                    uniqueItems.indexOf(uniqueItems.find((uniqueItem) => uniqueItem.number === item.number))
                ),
                uniqueItems
            );
        }
...
function _getRandomNumberExcluding(min, max, excludedNumbers = []) {
    ...
    if (excludedNumbers.includes(randomIndex)) {
        return _getRandomNumberExcluding(min, max, excludedNumbers);
    }
